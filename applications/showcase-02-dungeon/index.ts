...
class DungeonEngine {
            ...
            while (unconnected.length > 0) {
                ...
                for (let i = 0; i < connected.length; i++) {
                    ...
                    for (let j = 0; j < unconnected.length; j++) {
                        ...
                        const dist = Math.abs(roomA.cx - roomB.cx) + Math.abs(roomA.cy - roomB.cy);
                        if (dist < bestDist) {
                            bestDist = dist;
                            bestConnIdx = i;
                            bestUnconnIdx = j;
    ...
    private fits(x: number, y: number, w: number, h: number): boolean {
        ...
        if (x - 1 < 1 || y - 1 < 1 || x + w >= this.w - 1 || y + h >= this.h - 1) return false;
    ...
    private dig(x: number, y: number, w: number, h: number): void {
        this.rooms.push({ x, y, w, h, cx: (x + (x + w - 1)) >> 1, cy: (y + (y + h - 1)) >> 1, connections: 0 });
    ...
    private doors(): void {
        for (let y = 1; y < this.h - 1; y++) {
            for (let x = 1; x < this.w - 1; x++) {
                ...
                const vert = l === Tile.Wall && r === Tile.Wall && walk(u) && walk(d) &&
                    (isFloor(u) || isFloor(d));
                // Horizontal passage: walls up & down, walkable left & right, at least one Floor
                const horz = u === Tile.Wall && d === Tile.Wall && walk(l) && walk(r) &&
                    (isFloor(l) || isFloor(r));
    ...
    private stripBulk(): void {
        const walkable = (t: Tile) => t === Tile.Floor || t === Tile.Corridor || t === Tile.Door;
            ...
            for (let x = 0; x < this.w; x++) {
                ...
                for (let dy = -1; dy <= 1 && !keep; dy++) {
                    for (let dx = -1; dx <= 1 && !keep; dx++) {
                        ...
                        if (nx >= 0 && nx < this.w && ny >= 0 && ny < this.h && walkable(this.grid[ny][nx])) {
                            keep = true;
                        }
                    }
                }
...
export class DungeonApp implements IApplication<Engine, User<any>> {
    ...
    initUser(_runtime: IRuntime, _engine: Engine, user: User<any>): void {
        // Create Display and configure 3 Z-Layers.
        ...
        const mapLayer = new Layer(new Vector2(0, 0), 0, DISPLAY_W, DISPLAY_H, { mustBeReliable: true });
        // Z1: Dynamic Entities (Player, Loot, Trails) (Unreliable for speed)
        const entityLayer = new Layer(new Vector2(0, 0), 1, DISPLAY_W, DISPLAY_H, { mustBeReliable: false });
        // Z2: Static UI (Reliable)
        const uiLayer = new Layer(new Vector2(0, 0), 2, DISPLAY_W, DISPLAY_H, { mustBeReliable: true });
        ...
        reg.defineButton(0, 'UP', [{ sourceId: 1, type: InputDeviceType.Keyboard, key: KeyboardInput.ArrowUp },
        { sourceId: 2, type: InputDeviceType.Keyboard, key: KeyboardInput.KeyW }]);
        reg.defineButton(1, 'DOWN', [{ sourceId: 3, type: InputDeviceType.Keyboard, key: KeyboardInput.ArrowDown },
        { sourceId: 4, type: InputDeviceType.Keyboard, key: KeyboardInput.KeyS }]);
        reg.defineButton(2, 'LEFT', [{ sourceId: 5, type: InputDeviceType.Keyboard, key: KeyboardInput.ArrowLeft },
        { sourceId: 6, type: InputDeviceType.Keyboard, key: KeyboardInput.KeyA }]);
        reg.defineButton(3, 'RIGHT', [{ sourceId: 7, type: InputDeviceType.Keyboard, key: KeyboardInput.ArrowRight },
        ...
        user.data = { dungeon, mapLayer, entityLayer, uiLayer, lastMove: 0, chunkX, chunkY };
    }

    private drawMap(engine: DungeonEngine, layer: Layer, chunkX: number, chunkY: number): void {
        ...
        layer.setOrders([
            ...
            OrderBuilder.bitmask16(0, MAP_OFFSET_Y, SCREEN_W, SCREEN_H, mask, [
                ...
                { char: TILE_VIS[Tile.Corridor].ch, fg: TILE_VIS[Tile.Corridor].fg, bg: 0 },
                { char: TILE_VIS[Tile.Door].ch, fg: TILE_VIS[Tile.Door].fg, bg: 0 },
                { char: TILE_VIS[Tile.LockedDoor].ch, fg: TILE_VIS[Tile.LockedDoor].fg, bg: 0 },
            ]),
        ]);
        layer.commit();
    }
    ...
    updateUser(_runtime: IRuntime, _engine: Engine, user: User<any>): void {
        ...
        for (let i = 0; i < dg.trail.length; i++) {
            ...
            if (tx >= 0 && tx < SCREEN_W && ty >= 0 && ty < SCREEN_H) {
                ...
                if (tr.life < 5) c = 10;
                if (tr.life < 3) c = 11;
                trailDots.push({ x: tx, y: MAP_OFFSET_Y + ty, char: '+', fg: c, bg: 0 });
            }
        }
        ...
        uo.push(OrderBuilder.text(DISPLAY_W - info.length - 2, 1, info, 8, 7));

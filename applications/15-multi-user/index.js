 ...
 */
import { Layer, Display, OrderBuilder, Vector2, KeyboardInput, InputDeviceType, } from "@primitiv/engine";
export class MultiUserShowcase {
    // 1. GLOBAL STATE
    ...
    update(_runtime, _engine) {
        const state = this.globalState;
        state.tickCount++;
        state.npcX += state.npcDirX;
        state.npcY += state.npcDirY;
        // Bounce on boundaries
        if (state.npcX <= 0 || state.npcX >= 79)
            state.npcDirX *= -1;
        if (state.npcY <= 5 || state.npcY >= 44)
            state.npcDirY *= -1; // Top 5 reserved for UI
    }
    // ==========================================
    // THE USER INITIALIZATION
    // ==========================================
    initUser(_runtime, _engine, user) {
        const width = 80;
        const height = 45;
    ...
    updateUser(_runtime, _engine, user) {
    ...
    destroyUser(_runtime, _engine, user) {
        const data = user.data;
        if (data && data.id) {

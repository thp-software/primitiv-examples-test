...
export class BotAI {
  ...
  private static countEscapeRoutes(
    gameMap: GameMap,
    x: number,
    y: number,
    fireRange: number,
    ...
    const bombDanger = new Set<string>();
    bombDanger.add(`${x},${y}`);
    ...
    for (const dir of directions) {
      ...
      const existingHeat = heatMap.get(key) || 0;
      if (existingHeat >= 100) continue;
      ...
      let foundSafe = !bombDanger.has(key);

      while (queue.length > 0 && !foundSafe) {
        const current = queue.shift()!;

        for (const d of directions) {
          const nnx = current.x + d.dx;
          const nny = current.y + d.dy;
          const nkey = `${nnx},${nny}`;

          if (visited.has(nkey)) continue;
          if (!gameMap.isWalkable(nnx, nny)) continue;

          const heat = heatMap.get(nkey) || 0;
          if (heat >= 100) continue;

          visited.add(nkey);

          if (!bombDanger.has(nkey)) {
            foundSafe = true;
            break;
          }
